...
import '../bloc/itinerary_map_bloc.dart';
...
class _ItineraryMapScreenState extends State<ItineraryMapScreen> {
  ...
  void _fetchItinerary() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final blocState = context.read<ItineraryMapBloc>().state;
      ...
      if (blocState is! ItineraryMapInitial) return;
      ...
      if (itineraryId != null && itineraryId.isNotEmpty) {
        context
            .read<ItineraryMapBloc>()
            .add(FetchMapItineraryRequested(itineraryId));
      }
    });
  }
  ...
  Future<void> _updateMap(ItineraryMapSuccess state) async {
    ...
    if (widget.startLat != null && widget.startLng != null) {
      ...
      startPointName = widget.startName;
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<ItineraryMapBloc, ItineraryMapState>(
      listener: (context, state) {
        if (state is ItineraryMapSuccess) {
          _updateMap(state);
        }
      },
  ...
  Widget _buildBody(BuildContext context, ItineraryMapState state,
      TripPlanningState planState) {
    if (state is ItineraryMapLoading || state is ItineraryMapInitial) {
      return const Center(child: CircularProgressIndicator());
    }

    if (state is ItineraryMapFailure) {
  ...
  Widget _buildHeader(BuildContext context, ItineraryMapSuccess state,
      TripPlanningState planState) {
    final city =
              ...
              if (state.isReoptimizing)
                ...
                IconButton(
                  ...
                  onPressed: () async {
                    ...
                    if (context.mounted) {
                      context.read<ItineraryMapBloc>().add(
                          ReoptimizeMapItineraryRequested(state.itinerary.id));
                    }
                  },
                ),
            ],
  ...
  Widget _buildDayTabs(BuildContext context, ItineraryMapSuccess state,
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return GestureDetector(
            onTap: () {
              context.read<ItineraryMapBloc>().add(ChangeMapItineraryDay(day));
            },
            child: Container(
  ...
  Widget _buildMapSection(ItineraryMapSuccess state) {
    return Container(
      height: 420,
        ...
        children: [
          GoogleMap(
            onMapCreated: (controller) {
              ...
              final bloc = context.read<ItineraryMapBloc>().state;
              if (bloc is ItineraryMapSuccess) {
  ...
  void _fitCameraToCurrentPlaces(ItineraryMapSuccess state) async {
  ...
  Widget _buildDetailsSheet(BuildContext context, ItineraryMapSuccess state,
